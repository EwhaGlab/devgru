...
def plot_trajs_and_points_on_image_v2(
    ax: plt.Axes,
    img: np.ndarray,
    dataset_name: str,
    list_trajs: list,
...
def project_points_v2(
    ...
    Returns:
        ...
        the original project func is wrong... it is not following CV convention.
        It has mixed CV (right hand rule) and left handed rules (Unity style)...
        modified by hkm
    """
    batch_size, horizon, _ = xy.shape
    xyz = np.concatenate(
        [xy, 0 * np.ones(list(xy.shape[:-1]) + [1])], axis=-1
    )
#    print("in xyz:\n", xyz)
    # create dummy rotation and translation vectors
    rmat = np.array([[0,-1,0], [0,0,-1],[1,0,0]], dtype=float)
    (rvec, jacobian)=cv2.Rodrigues(rmat)
    tvec = (0, camera_height,-camera_x_offset)
    xyz_cv = xyz
    uv, _ = cv2.projectPoints(
        xyz_cv.reshape(batch_size * horizon, 3), rvec, tvec, camera_matrix, dist_coeffs
    )
...
def get_pos_pixels(
    ...
    pixels[:, 0] = VIZ_IMAGE_SIZE[0] - pixels[:, 0]  # <=== this shifts the left handed rule coord to right handed rule coord
    if clip:
        pixels = np.array(
            [
                [
