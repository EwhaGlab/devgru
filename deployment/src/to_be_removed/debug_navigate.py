...
BASE_DIR = '/home/hankm/python_ws/viznav/depth-nav'   #dirname(dirname(dirname(abspath(__file__))))
import sys
...
# UTILS
from topic_names import (DEPTH_TOPIC, #IMAGE_TOPIC,
                        WAYPOINT_TOPIC,
...
#def main(args: argparse.Namespace):
    ...
    model_config_path = model_paths['image_pose_rnn']["config_path"]
    with open(model_config_path, "r") as f:
    ...
    # load model weights
    ckpth_path = DEPLOYMENT_DIR+"/model_weights/latest.pth" #model_paths[args.model]["ckpt_path"]
    ...
    model.eval()
     ...
     # load topomap
    synced_images_all = os.listdir(os.path.join(topomap_dir))
    image_type = model_params['goal_type']
    topomap_img_files = [f for f in synced_images_all if image_type in f ]
     
    topomap_filenames = sorted(topomap_img_files, key=lambda x: int(''.join(filter(str.isdigit, x))) )
    tot_images = len(topomap_filenames)
    num_nodes = int( tot_images / subgoal_spacing )
    
    topomap = []
    for i in range(0, tot_images, subgoal_spacing):
    ...
    image_curr_msg = rospy.Subscriber(
        DEPTH_TOPIC, Image, callback_obs, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if len(context_queue) > model_params["context_size"]:
            if model_params["model_type"] == "nomad":
                print("Only image_pose_rnn for now.. I will look at the NODMAD later \n")
                raise NotImplementedError
            ...
            elif model_params["model_type"] == "image_pose_rnn":
                start = max(closest_node - 4, 0)  # default is 4
                end = min(closest_node + 4 + 1, goal_node)
                distances = []
                waypoints = []
                batch_obs_imgs = []
                batch_goal_data = []
                for i, sg_img in enumerate(topomap[start: end + 1]):
                    ...
                    obs_depth = context_queue
                    batch_obs_ds.append(obs_depth)
