...
def callback_curr_obs(msg):
    global curr_obs_rgb
    #print("got a rgb image")
    curr_obs_rgb = msg_to_pil(msg)
...
def synced_data_callback(depth_msg, odom_msg):
    global last_callback_time
    
    now = time.time()

    if (now - last_callback_time) < callback_fn_interval:
        return
    ...
    if len(context_depth_queue) < context_size + 1:
        context_depth_queue.append(obs_depth)
        context_odom_queue.append(odom)
        #context_rgb_queue.append(obs_rgb)
    else:
        context_depth_queue.pop(0)
        context_odom_queue.pop(0)
        context_depth_queue.append(obs_depth)
        context_odom_queue.append(odom)
...
# def update_correcting_htm( est_sg_pose = list, gt_sg_pose = list ):
#     xe, ye, th_e = est_sg_pose
#     xgt, ygt, th_gt = gt_sg_pose
#     wHe = rm.xyzrpy_to_htm( [xe, ye, 0, 0, 0, th_e]  )
#     wHgt = rm.xyzrpy_to_htm( [xgt, ygt, 0, 0, 0, th_gt]  )
#     gtHw = np.linalg.inv(wHgt)
#     gtHe = np.matmul( gtHw, wHe )
#     return gtHe  #  est wrt GT

# def odom_context_correction( context_odom_queue = list, correcting_htm = np.adarray ):
#     context_odom_queue
