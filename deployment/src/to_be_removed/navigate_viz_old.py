...
def load_caminfo():
    dataset_name = 'former'
    with open(os.path.join(BASE_DIR, "/data/data_config.yaml"), "r") as f:
...
def main(args: argparse.Namespace):
    ...
    sync = message_filters.ApproximateTimeSynchronizer([depth_sub, odom_sub], queue_size=10,
    ...
    while not rospy.is_shutdown():
        ...
        if len(context_depth_queue) > model_params["context_size"]:
            ...
            if model_params["model_type"] == "image_pose_rnn":  # (len(context_queue) > model_params["context_size"]):
                ...
                print(
                    "I am starting from <%d>th sg. Now, my curr sg idx is: <%d>, and the final goal node idx <%d>\n" % (
                    ...
                    for ii in range(0, obs_queue_size):
                        ...
                        if normalize:
                            pose_norm = _normalize_pose(np.array([rel_x, rel_y, rel_theta]),
                                                        waypoint_spacing=waypoint_spacing, eta=1.0,
                        ...
                        if learn_angle:
                            ...
                            assert round(math.sqrt(q_a[0] * q_a[0] + q_a[-1] * q_a[-1]),
                                         3) == 1.0, f"Is SG q_a: {q_a} unit quat ? "
