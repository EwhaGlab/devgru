...
from depth_nav_eval.evaluate_loop import (
    evaluate_loop,
    load_model,
)
...
def main(config):
    ...
    if "eval_batch_size" not in config:
        config["eval_batch_size"] = 32 #config["batch_size"]
    ...
    elif config["model_type"] == "image_rnn":
        if config["goal_type"] == "rgb":
            model = ImageRNN(
                context_size=config["context_size"],
                len_traj_pred=config["len_traj_pred"],
    ...
    ckpth_name = config['deployment']["ckpth_name"]
    #ckpth_path = MODEL_WEIGHTS_PATH + "/" + ckpth_name
    ckpth_path = '/home/hankm/python_ws/viznav/depth-nav/train/logs/depth-nav-release/depth-nav_2025_10_05_17_00_42/latest.pth'
    model = load_model(
        ckpth_path,
        config,
        device,
    ...
    if config["model_type"] == "image_nav" or config["model_type"] == "image_pose_rnn" or config["model_type"] == "image_pg_rnn":
        evaluate_loop(
            goal_type=config["goal_type"],
            model=model,
