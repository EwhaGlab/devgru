...
class DepthRNN(BaseModel):
    def __init__(
        ...
        if obs_encoder.split("-")[0] == "efficientnet":
            ...
            self.goal_encoder = EfficientNet.from_name("efficientnet-b0", in_channels=2) # goal and curr obs
        ...
        self.depth_rnn = GRUModel(  input_size  = self.obs_encoding_size,
    ...
    def forward(
        ...
        tuple_obs_depths = torch.split(obs_depths, 1, dim=1) # ( [B, 1, H, W], ..., [B, 1, H, W] )
        goal_concat = torch.concat( (tuple_obs_depths[-1], goal_depth), dim=1)
        ...
        obs_depths = torch.concat(obs_depths, dim=0) # [B*Contxt, 1, H, W]
        ...
        # gru model
        final_repr = self.depth_rnn( tokens )       # [B, 32]
