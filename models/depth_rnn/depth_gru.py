...
from depth_nav_train.models.depth_nav.gru_model import GRUModel
...
class DepthSeries(BaseModel):
    def __init__(
        ...
        """
        super(DepthSeries, self).__init__(context_size, len_traj_pred, learn_angle)
        ...
        if obs_encoder.split("-")[0] == "efficientnet":
            ...
            self.goal_encoder = EfficientNet.from_name("efficientnet-b0", in_channels=1) # goal
            self.num_goal_features = self.goal_encoder._fc.in_features  # 1280
        else:
            raise NotImplementedError
        
        if self.num_obs_features != self.obs_encoding_size:             # obs_encoding_size = 512
        ...
        self.depth_nav = GRUModel(input_size = self.obs_encoding_size,
        ...
        dist_pred_len = int( self.num_action_params / 2 )
    ...
    def forward(
        ...
        # goal_depth: [B, 1, H, W]
        goal_encoding = self.goal_encoder.extract_features(goal_depth)
        ...
        obs_encoding = self.obs_encoder.extract_features(obs_depths)
        ...
        # gru model
        final_repr = self.depth_nav( tokens )
