...
class DevGRU(BaseModel):
    def __init__(
        ...
        """
        super(DevGRU, self).__init__(context_size, len_traj_pred, learn_angle)
        ...
        self.dev_gru = GRUModel( input_size = self.obs_encoding_size,
                                        hidden_size= self.obs_encoding_size,
                                        seq_len    = self.context_size + 2, final_dim=final_dim)

        dist_pred_len = int( self.num_action_params ) #/ 2 )  [dx, dy, qw, qz]
        self.dist_predictor = nn.Sequential( nn.Linear(final_dim, dist_pred_len) )
    ...
    def forward(
        ...
        # gru model
        final_repr = self.dev_gru( tokens )       # [B, 32]
