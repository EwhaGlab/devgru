...
class ImagePGRNN(BaseModel):
    def __init__(
        ...
        """
        super(ImagePGRNN, self).__init__(context_size, len_traj_pred, learn_angle)
        ...
        self.image_pg_rnn = GRUModel(   input_size = self.obs_encoding_size,
    ...
    def forward(
        self, obs_images: torch.tensor, goal_image: torch.tensor, goal_pos: torch.tensor
        ...
        tokens = torch.cat((obs_encoding, goal_fused_token), dim=1)  # [B, 7, 512]
        
        # gru model
        final_repr = self.image_pg_rnn( tokens )       # [B, 32]
