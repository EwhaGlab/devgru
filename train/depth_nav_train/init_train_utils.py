...
# obs_rgb
        obs_images = torch.split(obs_rgb, 3, dim=0)  # tuple  of  obs rgb imgs   # was dim=1 which might be a bug
        viz_obs_image = TF.resize(obs_images[-1], VISUALIZATION_IMAGE_SIZE) # current obs img (160, 120)
        obs_images = [transform(obs_image).to(device) for obs_image in obs_images]
        obs_image = torch.cat(obs_images, dim=1)

# obs_depth
        viz_obs_depth = TF.resize(obs_depth, VISUALIZATION_IMAGE_SIZE)
        obs_depth = obs_depth.to(device)

# goal_rgb
        viz_goal_image = TF.resize(goal_rgb, VISUALIZATION_IMAGE_SIZE)
        goal_image = transform(goal_rgb).to(device) # transform() does img normalization (data preprocessing)

# goal_depth
        viz_goal_depth = TF.resize(goal_depth, VISUALIZATION_IMAGE_SIZE)
        goal_depth = goal_depth.to(device)

        model_outputs = model(obs_image, obs_depth, goal_image, goal_depth)

        dist_label = dist_label.to(device)          # 0~20  temporal distance to goal ?
        action_label = action_label.to(device)      # [5, 4]  4 actions (x,y, c, s) for 5 future steps
        ...
        _log_data(
            ...
            obs_depth= viz_obs_depth,
            goal_image=viz_goal_image,
            goal_depth=viz_goal_depth,
            action_pred=action_pred,
            action_label=action_label,
            dist_pred=dist_pred,
            dist_label=dist_label,
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        # goal_rgb
            ...
            goal_image = transform(goal_image).to(device)  # transform() does img normalization (data preprocessing)
