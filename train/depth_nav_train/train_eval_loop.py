...
from depth_nav_train.train_utils import train_lwf, train_coll, evaluate_coll, train, evaluate
...
def train_eval_loop_lwf(
    ...
    # # 1) freeze encoders
    # for p in model.obs_encoder.parameters():  p.requires_grad = False
    # for p in model.goal_encoder.parameters(): p.requires_grad = False
    # # 2) fix BN
    # model.apply(set_bn_eval)  # keep BN running stats fixed
    # model.apply(freeze_bn_affine)  # optional: also freeze BN gamma/beta

    for epoch in range(current_epoch, current_epoch + epochs):
        if train_model:
            print(
            f"Start DepthNav Training Epoch {epoch}/{current_epoch + epochs - 1}"
            )
            train_lwf(      #train(
                goal_type=goal_type,
                model=model,
        ...
        for dataset_type in test_dataloaders:
            print(
                f"Start {dataset_type} DepthNav Testing Epoch {epoch}/{current_epoch + epochs - 1}"
            )
