...
from data.depth_nav_dataset import DepthNav_Dataset
...
#    transform = ([transforms.Normalize(mean=[1, 1, 1], std=[0, 0, 0]),])
    ...
    # Create the model
    if config["model_type"] == "image-pose-rnn":
    ...
    elif config["model_type"] == "image-rnn":
        if config["goal_type"] == "rgb":
            model = ImageRNN(
                context_size=config["context_size"],
                len_traj_pred=config["len_traj_pred"],
    ...
    if config["model_type"] == "image-nav" or config["model_type"] == "image-pose-rnn":
        train_eval_loop(
            goal_type=config["goal_type"],
