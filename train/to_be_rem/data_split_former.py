...
def main(args: argparse.Namespace):
    ...
    bag_dirs = glob.glob('%s/bag_*'%base_dir)
    bag_dirs.sort()
    for bag_dir in(bag_dirs):

        pos_dir = '%s/synced'% bag_dir
        img_dir = '%s/synced'% bag_dir
        pose_file = '%s/sync_odom.txt'%pos_dir
        ...
        pkl_file = '%s/traj_data.pkl'%pos_dir

        for idx in range(0, num_pose):
            xy = pose_dat[idx][4:6].astype('float')
            quat = pose_dat[idx][[10,7,8,9]].astype('float') # quat, [qw, qx, qy, qz]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-splits-dir", "-o", default="/home/hankm/python_ws/viznav/data/data_splits", help="Data splits directory"
    )
